...
AimPoint Aimer::choose_aim_point(const Target & target)
{
  ...
  if (std::abs(target.ekf_x()[8]) <= 2 && target.name != ArmorName::outpost) {
    ...
    // 锁定模式
    if (id_list.size() > 1) {
      std::size_t id0 = id_list[0], id1 = id_list[1];
      if (lock_id_ != static_cast<double>(id0) && lock_id_ != static_cast<double>(id1))
        lock_id_ = (std::abs(delta_angle_list[id0]) < std::abs(delta_angle_list[id1])) ? id0 : id1;

      return {true, armor_xyza_list[static_cast<std::size_t>(lock_id_)]};
    }
