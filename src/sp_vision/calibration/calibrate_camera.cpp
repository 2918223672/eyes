...
const std::string keys =
  ...
  "{input-folder   | assets/chessboard        | 输入文件夹路径   }"
  "{@input_pos     |                          | 位置参数（可选） }";
  ...
  std::vector<std::vector<cv::Point2f>> & img_points)
{
  ...
  for (int i = 1; true; i++) {
    ...
    auto success = cv::findChessboardCorners(gray, pattern_size, corners_2d);
    ...
    // 输出识别结果
    if (success && !corners_2d.empty()) {
      fmt::print(
        "[success] {} ({} pts, x:{:.0f}-{:.0f} y:{:.0f}-{:.0f})\n",
        img_path, corners_2d.size(),
        corners_2d.front().x, corners_2d.back().x,
        corners_2d.front().y, corners_2d.back().y);
    } else {
      fmt::print("[failure] {}\n", img_path);
    }
    if (!success) continue;
...
int main(int argc, char * argv[])
{
  ...
  if (input_folder.empty()) input_folder = cli.get<std::string>(0);
